  ...
  City.findById(req.params.id, (err, city) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Nenhuma cidade foi encontrada pelo id ${req.params.id} informado.`
        });
      } else {
        res.status(500).send({
...
//Encontra a cidade pelo nome informado
exports.findCityByName = (req, res) => {
  City.find(req.params.name, (err, city) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Nenhuma cidade foi encontrada pelo nome ${req.params.name} informado.`
        });
      } else {
        res.status(500).send({
          message: `Um erro ocorreu durante a recuperação da cidade pelo nome ${req.params.name} informado.`
        });
      }
    } else res.send(city)
  });
...
//Encontra a cidade pelo nome estado informado
exports.findCityByState = (req, res) => {
  City.findOne(req.params.state, (err, city) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Nenhuma cidade foi encontrada pelo estado ${req.params.state} informado.`
        });
      } else {
        res.status(500).send({
          message: `Um erro ocorreu durante a recuperação da cidade pelo estado ${req.params.state} informado.`
        });
      }
    } else res.send(city)
  });
